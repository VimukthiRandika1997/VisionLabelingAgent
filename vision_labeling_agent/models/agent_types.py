# Extracted from: https://github.com/landing-ai/vision-agent/blob/main/vision_agent/models/agent_types.py
...
class AgentMessage(BaseModel):
    """
    This ecompasses messages sent to the entire Agentic system including
    both LLMs and sub-agents.

    user: The user's message.
    assistant: The assistant's message.
    observation: An observation made after conducting an action, either by the user or
                assistant.
    interaction: An interaction between the user and the assistant.
                ex: if the assistant wants to ask the user for help on a task,
                    it could send an interaciton message.
    conversation: Messages coming from the conversation agent, this is a type of
                assistant messages.
    planner: Messages coming from the planner agent, this is a type of assistant messages.
    ...
    role: Union[
        ...
        Literal["planner"],
        Literal["planner_update"],      # an intermediate update from the planner to show partial information
        Literal["coder"],
    ]
    content: str
...
class ErrorContext(BaseModel):
    ...
    These errors can happen during planning phase when a model does not output correctly
    formatted messages ( often time due to some guardtrails) 
